...
run_scenario <- 
  function(scenario = 1,
    ...
    R0_t7 <- as.Date(x = "5/1/2021", format = "%m/%d/%Y")
    R0_t7b <- as.Date(x = "10/31/2021", format = "%m/%d/%Y")
    ...
    dates <- c(R0_t0, R0_t1, R0_t2, R0_t3, R0_t4, R0_t5, R0_t6, R0_t7, R0_t7b, R0_t8, R0_t9)
    
    # compute level of Rt reached before Omicron switch
    rt <-    c(2.5,   2.5,   0.8,   0.8,   1.05,   1.05,   1.0,   1.0,  max_Rt, max_Rt, max_Rt_omicron)
