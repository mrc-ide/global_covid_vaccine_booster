...
summary_df <- df %>%
  ...
  mutate(value = `1065` - `334`) %>%
  group_by(income_group, target_pop, max_coverage, age_groups_covered, age_groups_covered_d3, vaccine_doses, vacc_start, strategy_name, vacc_per_week, t_d3, dose_3_timing, rollout_rate, compartment, age) %>%
